...
class MainMenuFrame(BaseFrame):
    ...
    def resize_widgets(self) -> None:
        ...
        if hasattr(self.app, '_get_widget_size'):
            ...
            btn_h = self.active_widgets.get('base_button_height', WindowConfig.DEFAULT_BUTTON_HEIGHT)
        
        # Update button sizes
        for button in [self.play_button, self.manage_button, self.quit_button]:
            if button and button.winfo_exists():
                button.configure(width=btn_w // 10, height=btn_h // 20)  # Adjust for character units
