...
class BaseDialog(tk.Toplevel, ABC):
    ...
    def _create_ui(self) -> None:
        ...
        self.content_frame = tk.Frame(self.main_frame, bg=ColorConfig.SECONDARY_BG)
        self.content_frame.pack(fill="both", expand=True, pady=(0, WindowConfig.PADDING_LARGE))

        # Button frame
        self.button_frame = tk.Frame(self.main_frame, bg=ColorConfig.SECONDARY_BG)
    ...
    def build_buttons(self) -> None:
        ...
        cancel_button = self.create_button(
            self.button_frame, "Cancel", self.cancel, style="secondary"
        )
        cancel_button.pack(side="right", padx=(WindowConfig.PADDING_SMALL, 0))

        # OK button
        ok_button = self.create_button(self.button_frame, "OK", self.ok, style="primary")
    ...
    def create_button(
        self, parent: tk.Widget, text: str, command: Callable[[], None], style: str = "primary"
    ) -> tk.Button:
        """
    ...
    def create_entry(self, parent: tk.Widget, placeholder: str = "", width: int = 30) -> tk.Entry:
        """
...
class MessageDialog(BaseDialog):
    ...
    def __init__(
        self, parent: tk.Widget, title: str, message: str, buttons: list = None, icon: str = None
    ):
        """
    ...
    def build_content(self) -> None:
        ...
        message_label = self.create_label(self.content_frame, self.message, font_size="label")
...
class InputDialog(BaseDialog):
    ...
    def build_content(self) -> None:
        ...
        # Prompt label
        prompt_label = self.create_label(self.content_frame, self.prompt, font_size="label")
        prompt_label.pack(pady=(0, WindowConfig.PADDING_MEDIUM))

        # Input entry
        self.entry = self.create_entry(self.content_frame, placeholder=self.placeholder, width=40)
